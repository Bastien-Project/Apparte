...
if ($lieuKey && isset($lieux[$lieuKey])) {
    ...
    echo "<h2>{$lieu['label']}</h2>";
    echo '<div class="lieu-detail"><a href="chambre.php" class="btn">← Retour à la sélection</a></div>';
} else {
    ...
    foreach ($lieux as $ville => $lieu) {
        ...
        echo '<h3>' . htmlspecialchars($lieu['label']) . '</h3>';
        echo '<img src="' . htmlspecialchars($lieu['images'][0]) . '" alt="' . htmlspecialchars($lieu['alt'][0]) . '">';
        echo '<p>' . htmlspecialchars($lieu['description'][0]) . '</p>';
